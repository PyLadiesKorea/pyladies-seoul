...
2. **Test-first development (TDD)**: Always begin implementation by writing tests for new functionality.
   - Write a failing test before writing production code
   - Follow the Red-Green-Refactor cycle: write a test (Red), implement minimal code to pass (Green), refactor as needed
   - Do not write production code unless there is a failing test

3. **Environment preparation**: Set up the implementation environment.
...
4. **Sequential implementation**: Execute each step in the provided order.
...
5. **Continuous validation**: Test and verify each step as you go.
...
6. **Integration and testing**: Ensure the completed task works correctly.
   - Run all specified tests (unit, integration, etc.)
   - Verify the component integrates properly with existing code
   - Test error handling and edge cases
   - Confirm all success criteria are met

7. **Documentation and cleanup**: Finalize the implementation.
...
2. **Test-first implementation (TDD)**: Always start implementation by writing tests for new features or bug fixes.
   - Do not write production code unless there is a failing test that requires it
   - Follow the Red-Green-Refactor cycle for every new functionality
   - Ensure all new code is covered by automated tests

3. **Write high-quality code**: Produce clean, maintainable, and well-structured code.
...
4. **Test thoroughly**: Validate each component as you implement it.
...
5. **Use appropriate tools**: Leverage all available tools effectively.
...
6. **Maintain working state**: Keep the system in a working state throughout implementation.
...
7. **Problem-solving approach**: Handle issues systematically when they arise.
